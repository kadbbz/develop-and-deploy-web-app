...
app.get(\`\${apiBase}/health\`, (_req, res) => {
  res.json({ ok: true, sessionId, token, basePath });
});
...
function scaffoldProject(meta) {
  const appDir = appRoot(meta.sessionId, meta.token);
  const basePath = `/${meta.sessionId}/${meta.token}`;
...
function main() {
  ...
  const meta = readJsonIfExists(path.join(appRoot(sessionId, token), "APP-META.json"), null);
  if (!meta) {
